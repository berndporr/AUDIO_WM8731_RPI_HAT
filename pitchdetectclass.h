...
public:
  ...
  tuple<double, double> detect (double a[]) {
    ...
    /** @brief Set mirror of the fft plot to zero as this is not needed */
    for(i = N/2; i < N; i++)
    {
      a[i] = 0;
    }

    /** @brief Set all values before 50Hz to zero as these may be detected as a maximum peak for the fft and wrongly assigned to be the fundamental frequency */
    for(i = 0; i < 21; i++)
    {
      a[i] = 0;
    }

    /** @brief Basic pitch detection algorithm which detects the highest peak of the fft and assumes that this is the fundamental frequency of the sound being recorded */
    for (i = 0; i < N; i++)
    {
    ...
    /** @brief Converts the fundamental frequency to Hz **/
     freqhz = freq * 44100 / N;

    /** @brief Return the maximum value of the fft and the fundamental frequency in Hz */
